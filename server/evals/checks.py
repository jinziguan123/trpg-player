...
from app.ai.turn_validator import _ANTITHESIS_RE, _INTERNAL_ID_RE, _REPORT_STYLE_RE
...
# 否定式对比句式（"不是X，是Y" / "不是X而是Y" / "与其说…不如说…"）过度复用探针。
# 正则真源在 app.ai.turn_validator._ANTITHESIS_RE（生产侧 nudge 与本探针共用同一份，防漂移）；
# 一场用一两次是点睛，密集复用则空洞、审美疲劳，故只测「过度复用」（一轮 ≥ 阈值次）。
